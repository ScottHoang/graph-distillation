...
from .res_tricks import DenseConnection
from .res_tricks import InitialConnection
from .res_tricks import ResidualConnection
...
class TricksComb(nn.Module):
    ...
    def __init__(self, args):
        ...
        if not self.has_residual_MLP:
            self.layers_GCN.append(
                GCNConv(self.num_feats,
                        self.dim_hidden,
                        cached=self.cached,
                        args=self.args,
                        whetherHasSE=self.args.TeacherGNN.whetherHasSE[0]))

        for i in range(self.num_layers):
            if (not self.has_residual_MLP) and (
                    0 < i < self.num_layers - 1
            ):  # if don't want 0_th MLP, then 0-th layer is assigned outside the for loop
                self.layers_GCN.append(
                    GCNConv(self.dim_hidden,
                            self.dim_hidden,
                            cached=self.cached,
                            args=self.args,
                            whetherHasSE=self.args.TeacherGNN.whetherHasSE[1]))
            ...
            appendNormLayer(
                self, args, self.dim_hidden
                if i < self.num_layers - 1 else self.num_classes)
            ...
            if AcontainsB(self.type_trick, ['Residual']):
                ...
                if self.layer_agg in ['concat', 'maxpool']:
                    self.layers_res.append(
                        DenseConnection((i + 2) * self.dim_hidden,
                                        self.dim_hidden, self.layer_agg))
                elif self.layer_agg == 'attention':
                    self.layers_res.append(
                        DenseConnection(self.dim_hidden, self.dim_hidden,
                                        self.layer_agg))
        ...
        if not self.has_residual_MLP:
            self.layers_GCN.append(
                GCNConv(self.dim_hidden,
                        ...
                        cached=self.cached,
                        args=self.args,
                        whetherHasSE=self.args.TeacherGNN.whetherHasSE[2]))

        if AcontainsB(self.type_trick, ['Jumping']):
            if self.layer_agg in ['concat', 'maxpool']:
                self.layers_res.append(
                    DenseConnection((self.num_layers + 1) * self.dim_hidden,
                                    self.num_classes, self.layer_agg))
            elif self.layer_agg == 'attention':
                self.layers_res.append(
                    DenseConnection(self.dim_hidden, self.num_classes,
                                    self.layer_agg))
        else:
            self.layers_MLP.append(nn.Linear(self.dim_hidden,
                                             self.num_classes))
    ...
    def forward(self, x, edge_index, want_les=False):
        ...
        for i in range(self.num_layers):
            ...
            beta = math.log(self.lamda / (i + 1) + 1) if AcontainsB(
                self.type_trick, ['IdentityMapping']) else self.lamda
...
class GCNConv(nn.Module):
    ...
    def __init__(self,
        ...
        if bias:
            if isinstance(out_feats, int):
                out_feats = th.Tensor(out_feats)
            out_feats = out_feats.float()

            self.bias = nn.Parameter(out_feats)
        else:
        ...
        if whetherHasSE:
            self.le = nn.Parameter(th.randn(args.N_nodes, self._out_feats),
                                   requires_grad=True)
