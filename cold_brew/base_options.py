...
class BaseOptions():
    ...
    def get_arguments(self):
        ...
        parser = argparse.ArgumentParser(
            description='Tail and cold start generalization')
        ...
        parser.add_argument(
            ...
            default=0,
            help=
            "This arguments controls the featureless mode. If set to 0, no change to original GNN; otherwise, discard input features, and use learnable embeddings of the specified dimension."
        )
        parser.add_argument(
            '--unify_mlps',
            type=int,
            default=0,
            help=
            "auxiliary function for batch training: if set to True, reset the MLP type methods' coefficient."
        )
        parser.add_argument('--force_set_to_best_config',
                            type=int,
                            default=1,
                            help="set dataset dependent configs.")
        parser.add_argument(
            '--want_headtail',
            type=int,
            default=1,
            help="wheter to add head and tail evaluation results as output.")
        parser.add_argument('--num_layers',
                            type=int,
                            default=2,
                            help="used for TeacherGNN")
        parser.add_argument(
            '--studentMLP__skip_conn_T_and_res_blks',
            type=str,
            default='',
            help=
            "architecture options for arch search of studentMLP. Use default is not doing architecture search."
        )
        ...
        parser.add_argument(
            ...
            default='',
            help=
            "optimization configuration for the studentMLP, better use default."
        )
        parser.add_argument(
            '--LP__which_corr_and_DAD',
            type=str,
            default='',
            help="two hyperpatameters in label propagation; better use default."
        )
        parser.add_argument(
            '--LP__num_propagations',
            type=int,
            default=-1,
            help="number of propagations in label propagation.")
        parser.add_argument('--LP__alpha',
                            type=float,
                            default=-1,
                            help="the alpha coefficient for label propagation")
        parser.add_argument(
            "--SEMLP_topK_2_replace",
            type=int,
            default=2,
            help=
            "the hyper parameter used to replace with the top K best neighbors"
        )
        parser.add_argument(
            "--SEMLP__include_part1out",
            type=int,
            default=1,
            help=
            "whether the part1 of cold brew's MLP is concatenated during part 2 training."
        )
        parser.add_argument(
            "--dropout_MLP",
            type=float,
            default=0.2,
            help=
            "dropout rate for Cold Brew MLP (both part1 and part2) and StudentBaseMLP modules."
        )
        parser.add_argument("--SEMLP_part1_arch",
                            type=str,
                            default='2layer',
                            choices=['residual', '2layer', '3layer', '4layer'],
                            help="architecture of part1 of Cold Brew MLP")
        parser.add_argument(
            '--has_proj2class',
            type=int,
            default=0,
            help="whether cold brew's TeacherGNN has additional projection head"
        )
        parser.add_argument(
            "--whetherHasSE",
            type=str,
            default='000',
            choices=['100', '001', '111', '000'],
            help="whether cold brew's TeacherGNN has structural embedding.")
        parser.add_argument(
            "--se_reg",
            type=float,
            default=10,
            help=
            "regularization coefficient for cold brew's structural embedding")
        parser.add_argument("--graphMLP_reg",
                            type=float,
                            default=0.,
                            choices=[0., 1., 10., 100.],
                            help="regularization coefficient in GraphMLP")
        parser.add_argument("--graphMLP_tau",
                            type=float,
                            default=2.0,
                            choices=[0.5, 1.0, 2.0],
                            help="the coefficient tau in GraphMLP")
        parser.add_argument(
            "--graphMLP_r",
            type=int,
            default=3,
            choices=[2, 3, 4],
            help=
            "the coefficient r (number of r-hop neighbors to consider) in GraphMLP"
        )
        parser.add_argument("--change_to_featureless",
                            type=int,
                            default=0,
                            help="whether switch to featureless graph.")
        parser.add_argument(
            "--do_deg_analyze",
            type=int,
            default=1,
            help=
            "if True, analyze graph data. has to be true, otherwise 'large_deg_mask' is not assigned and will bug."
        )
        parser.add_argument("--train_which",
                            type=str,
                            default='TeacherGNN',
                            choices=[
                                'TeacherGNN', 'SEMLP', 'LP', 'StudentBaseMLP',
                                'GraphMLP', 'proj2class'
                            ])
        ...
        parser.add_argument("--dataset",
                            ...
                            choices=[
                                ...
                                'chameleon', 'ACTOR', 'squirrel', 'WISCONSIN',
                                'CORNELL', 'TEXAS'
                            ])
        parser.add_argument("--use_special_split", type=int, default=1)
        parser.add_argument('--optfun',
                            type=str,
                            default='torch.optim.Adam',
                            choices=['torch.optim.Adam', 'torch.optim.SGD'])
        parser.add_argument('--manual_assign_GPU',
                            type=int,
                            default=-9999,
                            help="default=-9999, means to choose bestGPU")
        ...
        parser.add_argument(
            '--records_desc',
            type=str,
            default='res_connection',
            help=
            "file name of training records, try to make your exp settings directly readable from this name, e.g., gcn_PairNorm"
        )
        parser.add_argument('--records_path',
                            type=str,
                            default='.',
                            help="saving location of training records")
        parser.add_argument(
            '--compare_model',
            type=int,
            default=0,
            help=
            "0 means compare single trick, 1 means compare model, 2 means compare trick combinations"
        )
        parser.add_argument('--type_model',
                            type=str,
                            default="GCN",
                            choices=[
                                'GCN', 'GAT', 'SGC', 'GCNII', 'DAGNN',
                                'GPRGNN', 'APPNP', 'JKNet', 'DeeperGCN'
                            ])
        parser.add_argument(
            '--type_trick',
            type=str,
            default='Initial+BatchNorm',
            help=
            "type of residual/dropout/normalization trics used in the TeacherGNN of Cold Brew. Can be one of ['Initial', 'Jumping', 'Residual', ''] + one of ['GroupNorm', 'BatchNorm', 'NoNorm']"
        )
        parser.add_argument('--layer_agg',
                            type=str,
                            default='concat',
        ...
        parser.add_argument('--layerwise_dropout',
                            action='store_true',
                            default=False)

        # ------- build up the parameters for link prediction transfer learning (I2-GTL) -------
        parser.add_argument('--public_data_convert_overlapped_subgraph',
                            type=bool,
                            default=True)
        parser.add_argument('--transfer_setting',
                            type=str,
                            default='i2t',
                            choices=['t2t', 'u2t', 'i2t', 'u', 'i', ''])
        parser.add_argument('--linkpred_baseline',
                            type=str,
                            default='',
                            choices=['', 'EGI', 'DGI'])
        parser.add_argument('--edge_lp_mode',
                            type=str,
                            default='logit',
                            choices=['emb', 'logit', 'xmc', ''])
        parser.add_argument('--ELP_alpha', type=str, default=0.995)
        parser.add_argument('--num_propagations', type=str, default=5)
        parser.add_argument('--LP_device',
                            type=str,
                            default='cuda:4',
                            choices=['cpu', 'cuda:0', 'cuda:4'])
        parser.add_argument('--exp_on_cold_edge', type=bool, default=False)
        parser.add_argument('--encoder',
                            type=str,
                            default='SAGE',
                            choices=['SAGE', 'MLP', 'CN', 'AA', 'PPR'])
        ...
        parser.add_argument(
            ...
            type=str,
            default='ce_loss',
            choices=['AUC', 'ce_loss', 'log_rank_loss', 'info_nce_loss'])
        parser.add_argument('--neg_sampler', type=str, default='global')
        parser.add_argument('--data_name',
                            type=str,
                            default='ogbl-citation2',
                            choices=['ogbl-citation2', 'ogbl-collab'])
        parser.add_argument('--data_path', type=str, default='dataset')
        parser.add_argument('--eval_metric',
                            type=str,
                            default='recall_my@1.25',
                            choices=[
                                'hits', 'mrr', 'recall_my@0.8', 'recall_my@1',
                                'recall_my@1.25', 'recall_my@0'
                            ])
        ...
        parser.add_argument('--use_valedges_as_input',
                            type=str2bool,
                            default=True)
    ...
    def ini_records_saver(self, args):
        ...
        if os.path.exists(records_file):
            records_file_backup = os.path.join(args.records_path,
                                               args.records_desc + ' - backup')
            print(
...
def best_alpha_or_agg(args):
    ...
    if args.type_trick in ['Residual', 'Initial']:
        ...
        idx = alpha_matrix[idx_1[args.dataset], idx_2[args.type_model],
                           idx_3[args.num_layers], idx_4[args.type_trick]]
...
def bestGPU(gpu_verbose=False, **w):
    ...
    for ig, gpu in enumerate(Gpus):
        ...
        if gpu_verbose:
            print(
                f'gpu-{ig}:   Memory: {memUtil:.2f}%   |   load: {load:.2f}% ')
...
def set_labprop_configs(args):
    ...
    args.lpStep.fn = [
        'double_correlation_fixed',  # 'lpStep.fn' ONLY apply to 'with MLP' case; not applicable to LP-only case.
...
def force_set_to_best_config(args):
    ...
    if args.train_which in ['SEMLP', 'StudentBaseMLP', 'TeacherGNN']:
        args.best_config_performance = [
            86.9639468690702, 72.44, 75.96000000000001, 71.5367364154476,
            68.50877192982458, 31.947368421052637, 59.78866474543709,
            65.09803921568627, 61.08108108108108, 81.62162162162163
        ]
        TeacherGNN_arr1 = (2, 4, 8, 16, 64)
        TeacherGNN_arr2 = ('NoRes', 'Initial', 'Dense', 'Residual')
        TeacherGNN_arr3 = ('NoNorm', 'GroupNorm', 'BatchNorm', 'PairNorm',
                           'NodeNorm')
        best_config_TeacherGNN = np.array([[0, 0, 4], [0, 0, 1], [4, 1, 2],
                                           [2, 1, 2], [1, 1, 3], [0, 0, 2],
                                           [0, 1, 4], [1, 3, 0], [2, 3, 3],
                                           [2, 3, 1]])
    ...
    if args.train_which in ['SEMLP', 'StudentBaseMLP']:
        ...
        arr2 = (128, 256)
        arr3 = ('torch.optim.Adam&0.001', 'torch.optim.Adam&0.005',
                'torch.optim.Adam&0.02', 'torch.optim.SGD&0.005')
        best_config = np.array([[0, 1, 0], [0, 0, 0], [1, 0, 3], [1, 1, 0],
                                [2, 0, 0], [0, 1, 2], [2, 1, 2], [0, 1, 0],
                                [0, 1, 3], [0, 0, 2]])
