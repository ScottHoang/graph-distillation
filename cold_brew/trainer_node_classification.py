...
from MLP_model import GraphMLP
...
class trainer:
    ...
    def main(self):
        ...
        if self.args.do_deg_analyze:
            save_graph_analyze(self.args.N_nodes, self.data,
                               self.args.use_special_split)
    ...
    def run_pureLP(self):
        ...
        acc_train, acc_test = np.round(acc_train * 100,
                                       2), np.round(acc_test * 100, 2)
    ...
    def train_seMLP_part1(self, ):
        ...
        print('-' * 30, '\n         Start training part-1 of SEMLP\n',
              '-' * 30)
        self.seMLP = SEMLP(self.args, self.data,
                           self.teacherGNN).to(self.device)
        ...
        self.seMLP.teacherSE = lrn_targ = self.teacherGNN.model.model.collect_SE(
        ...
        for epoch in range(self.epochs):
            ...
            batch_idx_train = np.random.choice(self.seMLP.train_idx,
                                               self.args.batch_size)
            part1_out = self.seMLP.forward_part1(self.data.x,
            ...
            if epoch % 1 == 0:
                self.seMLP.eval()
                batch_idx_train = np.random.choice(self.seMLP.test_idx,
                                                   self.args.batch_size)
                part1_out = self.seMLP.forward_part1(self.data.x,
                                                     batch_idx=batch_idx_train)
                ...
                result = [
                    np.log(toitem(loss_train)),
                    np.log(toitem(loss_test))
                ]

                results_arr2D.append(result)
                if epoch % 20 == 0:
                    print(
                        f'epoch {epoch}, train/test loss {toitem(loss_train):.2}, {toitem(loss_test):.2}'
                    )

        save_model(self.seMLP, join(self.modeldir, 'seMLP-part-1'))
        results_arr2D = np.array(results_arr2D).T
        npy_dir = f'{self.resdir}/seMLP'
        wzRec(results_arr2D[0],
              f'loss_train@{npy_dir.replace("/","@")}',
              want_save_npy=1,
              npy_dir=npy_dir)
        wzRec(results_arr2D[1],
              f'loss_test@{npy_dir.replace("/","@")}',
              want_save_npy=1,
              npy_dir=npy_dir)
        results_arr2D_concise = results_arr2D[[
            1
        ]]  # shape = [1, epochs] ; only want acc_test
    ...
    def train_seMLP_part2(self, ):
        ...
        if self.args.SEMLP__downgrade_to_MLP:
            ...
            self.seMLP = SEMLP(self.args, self.data,
                               teacherGNN=None).to(self.device)
            self.seMLP.train_idx = self.seMLP.train_idx.to('cpu')
            self.seMLP.train_mask = self.seMLP.train_mask.to('cpu')
            self.seMLP.test_idx = self.seMLP.test_idx.to('cpu')
        ...
        for epoch in range(self.epochs):
            ...
            batch_idx_train = np.random.choice(self.seMLP.train_idx,
                                               self.args.batch_size)
            part2_out_train = self.seMLP.forward_part2(
                self.data.x,
            ...
            if epoch % 1 == 0:
                ...
                batch_idx_train = np.random.choice(self.seMLP.test_idx,
                                                   self.args.batch_size)
                part2_out_test = self.seMLP.forward_part2(
                    self.data.x,
                    edge_index=self.data.edge_index,
                    batch_idx=batch_idx_train)

                acc_test = evaluate(part2_out_test,
                                    self.data.y[batch_idx_train], None) * 100
                result = [toitem(acc_test)]

                if self.args.want_headtail:

                    batch_idx = self.data.large_deg_idx
                    train_head, test_head = self.eval_headtail__traintest_v2(
                        self.seMLP.forward_part2(
                            self.data.x,
                            edge_index=self.data.edge_index,
                            batch_idx=batch_idx), lrn_targ[batch_idx],
                        batch_idx, cal_acc_rounded100)

                    batch_idx = self.data.small_deg_idx
                    train_tail, test_tail = self.eval_headtail__traintest_v2(
                        self.seMLP.forward_part2(
                            self.data.x,
                            edge_index=self.data.edge_index,
                            batch_idx=batch_idx), lrn_targ[batch_idx],
                    ...
                    if self.args.use_special_split:
                        ...
                        train_iso, test_iso = self.eval_headtail__traintest_v2(
                            self.seMLP.forward_part2(
                                self.data.x,
                                edge_index=self.data.edge_index,
                                batch_idx=batch_idx), lrn_targ[batch_idx],
                ...
                if epoch % 20 == 0:
                    ...
                    print(
                        f'epoch {epoch}, acc test {toitem(acc_test):.2f}, head_tail_iso = {result[-3:]}'
                    ) if self.args.use_special_split else print(
                        f'epoch {epoch}, acc test {toitem(acc_test):.2}')

        save_model(self.seMLP, join(self.modeldir, 'seMLP'))
        results_arr2D = np.array(results_arr2D).T
        npy_dir = f'{self.resdir}/seMLP'
        wzRec(results_arr2D[0],
              f'acc_test@{npy_dir.replace("/","@")}',
              want_save_npy=1,
              npy_dir=npy_dir)

        figure()
        plot_many(results_arr2D[[0]], ['acc-test'],
                  ttl='#ALL2#__' + npy_dir.replace("/", "@"))
        if not self.args.want_headtail:
            results_arr2D_concise = results_arr2D[[
                0
            ]]  # shape = [1, epochs] ; only want acc_test
        else:
            results_arr2D_concise = results_arr2D[[0, -3, -2,
                                                   -1]]  # shape = [4, epochs]
    ...
    def eval_headtail__traintest(self, emb2, lrn_targ, metricfun):
        ...
        metric_head_train = metricfun(emb2[mask_head_train],
                                      lrn_targ[mask_head_train])
        metric_head_test = metricfun(emb2[mask_head_test],
                                     lrn_targ[mask_head_test])
        metric_tail_train = metricfun(emb2[mask_tail_train],
                                      lrn_targ[mask_tail_train])
        metric_tail_test = metricfun(emb2[mask_tail_test],
                                     lrn_targ[mask_tail_test])
        return [
            metric_head_train, metric_head_test, metric_tail_train,
            metric_tail_test
        ]
    ...
    def load_teacherGNN(self, keyw=''):
        if self.args.has_proj2class:
            self.proj2class = getMLP(
                self.args.TeacherGNN.neurons_proj2class).to(self.device)
        else:
            self.proj2class = None
        self.teacherGNN = TeacherGNN(self.args,
                                     self.proj2class).to(self.device)
        if 'best' in keyw:
    ...
    def save_config(self, args, savedir, ignore_keys):
        args = vars(args)
        configs = {}
        for k, v in args.items():
            if isinstance(
                    v,
                (int, str, float, bool, dict, argparse.Namespace, utils.D)):
                if k in ignore_keys:
                    continue
                if isinstance(v, argparse.Namespace):
                    configs[k] = vars(v)
                elif isinstance(v, utils.D):
                    configs[k] = v.__dict__
                else:
                    configs[k] = v
        with open(os.path.join(self.modeldir, 'config.txt'), 'w') as f:
    ...
    def __init__(self, args, which_run):
        ...
        if self.dataset == 'ogbn-arxiv':
            ...
            if not self.args.use_special_split:
                self.data.train_mask = torch.BoolTensor(
                    [False] * self.args.N_nodes).to(self.device)
                self.data.test_mask = torch.BoolTensor(
                    [False] * self.args.N_nodes).to(self.device)
                self.data.train_mask[self.train_idx] = True
                self.data.test_mask[self.split_idx['test'].to(
                    self.device)] = True
        else:
    ...
    def train_teacherGNN(self):
        ...
        self.save_config(self.args,
                         self.modeldir,
                         ignore_keys=['preStep', 'midStep', 'lpStep', 'data'])
        self.optimizer = self.optfun(self.teacherGNN.parameters(),
                                     lr=self.args.lr,
        ...
        for epoch in range(self.epochs):
            ...
            if keep_saved_teacher_model_strongest and acc_test > best_test_acc:
                ...
                save_model(self.teacherGNN,
                           join(self.modeldir, 'best-teacherGNN'))

            # ---- defining what results to return:TeacherGNN ----
            results_arr2D.append([
                np.log(loss_train), acc_train * 100, acc_test * 100,
                linkp_train, linkp_test
            ])
            if self.args.want_headtail:
            ...
            if epoch % 20 == 0:
                if self.args.has_loss_component_edgewise:
                    print(
                        f'Ep{epoch:03d}, linkp train/test mrr: {linkp_train:.3f} / {linkp_test:.3f}'
                    )
                else:
                    print(
                        f'Ep{epoch:03d}, acc @ train/test: {acc_train*100:.1f}, {acc_test*100:.1f} '
                    )

        print('train_loss: {:.4f},  test_acc:{:.4f}'.format(
            best_train_loss, best_test_acc))
        ...
        wzRec(results_arr2D[0],
              ...
              npy_dir=npy_dir)
        wzRec(results_arr2D[1],
              f'acc_train@{npy_dir.replace("/","@")}',
              want_save_npy=1,
              npy_dir=npy_dir)
        wzRec(results_arr2D[2],
              f'acc_test@{npy_dir.replace("/","@")}',
              want_save_npy=1,
              npy_dir=npy_dir)
        wzRec(results_arr2D[3],
              f'linkp_train@{npy_dir.replace("/","@")}',
              want_save_npy=1,
              npy_dir=npy_dir)
        wzRec(results_arr2D[4],
              f'linkp_test@{npy_dir.replace("/","@")}',
              want_save_npy=1,
              npy_dir=npy_dir)

        figure()
        plot_many(results_arr2D[[1, 2, 3, 4]], [
            'acc_train',
            'acc_test',
            'linkp_train',
            'linkp_test',
        ],
                  ttl='#ALL1#__' + npy_dir.replace("/", "@"))
        figure()
        plot_many(results_arr2D[[0]], ['log_loss_train'],
                  ttl='#ALL2#__' + npy_dir.replace("/", "@"))
        if not self.args.want_headtail:
            results_arr2D_concise = results_arr2D[[2]]  # shape = [1, epochs]
        else:
            results_arr2D_concise = results_arr2D[[2, -3, -2,
                                                   -1]]  # shape = [4, epochs]
        return results_arr2D_concise
    ...
    def run_trainSet(self):
        ...
        if self.args.has_loss_component_nodewise:
            ...
            loss_semantic = self.loss_fn(logits,
            ...
            result = []
            if self.args.want_headtail:
                all_node_logits = self.teacherGNN.get_3_embs(
                    self.data.x, self.data.edge_index).emb4classi
                lrn_targ = self.data.y

                batch_idx = self.data.large_deg_idx
                train_head, test_head = self.eval_headtail__traintest_v2(
                    all_node_logits[batch_idx], lrn_targ[batch_idx], batch_idx,
                    cal_acc_rounded100)
                ...
                if self.args.use_special_split:
                    batch_idx = self.data.zero_deg_idx
                    train_iso, test_iso = self.eval_headtail__traintest_v2(
                        all_node_logits[batch_idx], lrn_targ[batch_idx],
                        batch_idx, cal_acc_rounded100)
                    result.extend([test_iso])
        ...
        if self.args.has_loss_component_edgewise:
            ...
            loss_structure, linkp_train = self.getLinkp_loss_eva(
                emb4linkp, 'train')
            # ======= link prediction: eva =======
    ...
    def evaluate_linkp(self, model, mode):
        ...
        if mode == 'train':
            ...
            mask = ~self.data.train_mask
        emb4linkp = model.get_emb4linkp(self.data.x, self.data.edge_index,
                                        mask)  # return ALL nodes
    ...
    def run_testSet(self):
        ...
        with torch.no_grad():
            if self.args.train_which == 'GraphSAGE':
                ...
                res = self.teacherGNN.get_3_embs(self.data.x,
    ...
    def gen_pn_edges(self, nodes_emb, mode):
        ...
        if mode == 'train':
            valid_edge_mask = self.data.train_mask[
                self.data.edge_index[0]] * self.data.train_mask[
                    self.data.edge_index[1]]  # len = edge_index
            valid_edge_index = self.data.edge_index[:,
                                                    valid_edge_mask]  # len = num of true
        elif mode == 'test':
            test_mask = ~self.data.train_mask
            valid_edge_mask = test_mask[self.data.edge_index[0]] * test_mask[
                self.data.edge_index[1]]  # len = edge_index_bkup
        ...
        samp_edge_p_idx = np.random.choice(valid_edge_index.shape[1],
                                           samp_size_p)
    ...
    def my_negative_sampling(self, mode):
        ...
        if mode == 'train':
            ...
            samp_size_n_sub = max(
                samp_size_n // 4,
                50)  # do neg sample in small batches to prevend over flood
            while N_sampled < samp_size_n:
                edge_samp = negative_sampling(self.data.edge_index,
                                              num_neg_samples=samp_size_n_sub,
                                              force_undirected=True)
                fall_in_mask = self.data.train_mask[
                    edge_samp[0]] * self.data.train_mask[edge_samp[1]]
            ...
            while N_sampled < samp_size_n:
                edge_samp = negative_sampling(self.data.edge_index,
                                              num_neg_samples=samp_size_n_sub,
                                              force_undirected=True)
                fall_in_mask = ~(self.data.train_mask[edge_samp[0]] *
                                 self.data.train_mask[edge_samp[1]])
                edge_samp = edge_samp[:, fall_in_mask]
                N_sampled += edge_samp.size(1)
                sampled_all.append(edge_samp)
...
def load_data(dataset, which_run, self):
    ...
    if dataset in ["Cora", "Citeseer", "Pubmed"]:
        ...
        if dataset == 'Cora':
            N_nodes_train = 600
            data.train_mask = torch.tensor([True] * N_nodes_train + [False] *
                                           (self.args.N_nodes - N_nodes_train))
        ...
        data = WikipediaNetwork(path, dataset,
                                transform=T.NormalizeFeatures())[0]
        data = change_split(data, dataset, which_split=int(which_run // 10))
    else:
