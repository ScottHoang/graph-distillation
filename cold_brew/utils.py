...
def plot_dist(arr, ttl='', bins=100, saveFig_fname=''):
    ...
    value, loc, handle = plt.hist(x=arr,
                                  bins=bins,
                                  color='#0504aa',
                                  alpha=0.5,
                                  rwidth=1)
...
def init_split_edge_unified_impl(data, is_bipt=False):
    ...
    As = graphUtils.edge_index_to_A(data.edge_index,
                                    data.num_nodes,
    ...
    for ie, e in enumerate(tqdm(tonp(data.edge_index.T).tolist())):
        ...
        if hasattr(data, 'is_unique_in_targetG_edge_mask'):
            ...
            cond0 = (not data.is_unique_in_targetG_mask[e[0]]) and (
                not data.is_unique_in_targetG_mask[e[1]]
            )  # both nodes in source graph
    ...
    neg_edge_samp = negative_sampling(
        data.edge_index,
        num_nodes=(data.N_asin, data.N_kw) if is_bipt else data.num_nodes,
    ...
    if has_edge_attr:
        split_edge = {
            'train': {
                'edge': torch.tensor(p_edge_train),
                'edge_neg': torch.tensor(n_edge_train),
                'edge_feat': torch.tensor(p_edge_train_feat)
            },
            'valid': {
                'edge': torch.tensor(p_edge_valid),
                'edge_neg': torch.tensor(n_edge_valid),
                'edge_feat': torch.tensor(p_edge_valid_feat)
            },
            'test': {
                'edge': torch.tensor(p_edge_test),
                'edge_neg': torch.tensor(n_edge_test),
                'edge_feat': torch.tensor(p_edge_test_feat)
            },
        }
    else:
        split_edge = {
...
def _subgraph(subset, edge_index, edge_attr, relabel_nodes, num_nodes,
              return_edge_mask):
    ...
    if subset.dtype == bool:  # or subset.dtype == torch.uint8:
        ...
        if relabel_nodes:
            node_idx = torch.zeros(node_mask.shape[0],
                                   dtype=torch.long,
                                   device=device)
...
def down_sample_A(edge_index,
    ...
    def v1(edge_index, perm, edge_weight=None, num_nodes=None):
        ...
        As = graphUtils.edge_index_to_A(edge_index,
                                        num_nodes=num_nodes,
...
def down_sample_graph_with_node_perm(data,
                                     perm=None,
                                     drop_rate=0.9,
                                     do_remove_isolated_nodes=True):
    data.num_nodes = len(data.x)
    if perm is None:
        perm = np.array(
            sorted(
                np.random.choice(len(data.x),
                                 int(len(data.x) * (1 - drop_rate)),
                                 replace=False)))
    else:
        perm = tonp(perm)
    ...
    if hasattr(data, 'edge_attr') and data.edge_attr is not None:
        data.edge_index, data.edge_attr, e_mask = down_sample_A(
            data.edge_index,
            perm,
            edge_weight=data.edge_attr,
        ...
        data.edge_index, data.edge_weight, e_mask = down_sample_A(
            data.edge_index,
            perm,
        ...
        data.edge_index, _ = down_sample_A(data.edge_index,
                                           perm,
                                           num_nodes=data.num_nodes)
    ...
    if do_remove_isolated_nodes:
        ...
        data.edge_index, data.edge_attr, _mask = remove_isolated_nodes(
            data.edge_index, edge_attr=data.edge_attr, num_nodes=len(perm))
        perm = perm[_mask]
...
def get_A_from_relabeled_edge_index(data, subset_idx, subset_newID, num_nodes):
    ...
    if hasattr(data, 'edge_attr'):
        _edge_index, _edge_attr = subgraph_relabel(data.edge_index,
                                                   subset_idx,
                                                   subset_newID=subset_newID,
                                                   edge_attr=data.edge_attr,
                                                   return_edge_mask=False)
        A = graphUtils.edge_index_to_A(_edge_index,
                                       num_nodes=num_nodes,
                                       edge_weight=_edge_attr,
                                       want_tensor=1)
    elif hasattr(data, 'edge_weight'):
        _edge_index, _edge_weight = subgraph_relabel(
            data.edge_index,
            subset_idx,
            subset_newID=subset_newID,
        ...
        _edge_index, _ = subgraph_relabel(data.edge_index,
                                          subset_idx,
                                          subset_newID=subset_newID,
                                          edge_attr=None,
                                          return_edge_mask=False)
...
def cal_union(data1, data2, is_bipt=False):
        ...
        if N_asin_12 == ():
            ...
            for i, t in enumerate(kw1):
                if t in t_map_i_2:
                    ...
                    is_unique_in_targetG_mask[t_map_i_2[t] +
                                              num_asin_all] = False
                else:
                    idx_1_map_U.append(id_starting_graph2)
                    is_unique_in_targetG_mask.insert(id_starting_graph2, False)
    ...
    if is_bipt:
        idx_1_map_U, idx_2_map_U, is_unique_in_targetG_mask, texts, N_asin, N_kw = get_union_graphs_idx_mapU(
            data1.texts, data2.texts, (data1.N_asin, data2.N_asin))
        dataU.texts = texts
    else:
        idx_1_map_U, idx_2_map_U, is_unique_in_targetG_mask, texts, N_asin, N_kw = get_union_graphs_idx_mapU(
            data1.texts, data2.texts, ())

    dataU.num_nodes = dataU.N_nodes = max(idx_1_map_U.max(),
                                          idx_2_map_U.max()) + 1
    dataU.N_asin, dataU.N_kw, dataU.is_unique_in_targetG_mask = N_asin, N_kw, is_unique_in_targetG_mask

    # ---- check ----
    assert int(is_unique_in_targetG_mask.sum() + data1.N_nodes) == int(
        dataU.N_nodes)
    print(
        f'check N_asin, N_kw: previously = {(data1.N_asin, data1.N_kw)}, {(data2.N_asin, data2.N_kw)} \n\t  now = {(N_asin, N_kw)}'
    )

    # ---- compute x ----
    dataU.x = torch.zeros([dataU.num_nodes, data2.x.shape[1]],
                          dtype=data2.x.dtype,
    ...
    # ---- compute edge_index ----
    A1 = get_A_from_relabeled_edge_index(data1,
                                         subset_idx=torch.arange(
                                             data1.num_nodes,
                                             dtype=torch.long,
                                             device=data1.edge_index.device),
                                         subset_newID=idx_1_map_U,
                                         num_nodes=dataU.num_nodes)
    A2 = get_A_from_relabeled_edge_index(data2,
                                         subset_idx=torch.arange(
                                             data2.num_nodes,
                                             dtype=torch.long,
                                             device=data2.edge_index.device),
                                         subset_newID=idx_2_map_U,
                                         num_nodes=dataU.num_nodes)
...
def target_seeded_by_source(data1, data2, actually_do_addition=True):
    ...
    is_shared_1_idx, is_shared_1_idx_map2 = get_shared_node_idx_and_map(
        data1.texts, data2.texts)
    ...
    if actually_do_addition:
        ...
        A2 = graphUtils.edge_index_to_A(data2.edge_index,
                                        num_nodes=data2.num_nodes,
                                        edge_weight=data2.edge_attr,
                                        want_tensor=1)
        A1 = get_A_from_relabeled_edge_index(data1, is_shared_1_idx,
                                             is_shared_1_idx_map2,
                                             data2.num_nodes)
...
def down_sample_edge_index(edge_index,
                           perm,
                           edge_attr=None,
                           return_edge_mask=True):
    ...
    return subgraph_relabel(edge_index,
                            perm,
                            edge_attr=edge_attr,
                            return_edge_mask=return_edge_mask,
                            subset_newID=None)
...
def subgraph_relabel(edge_index,
                     subset_idx,
                     subset_newID=None,
                     edge_attr=None,
                     return_edge_mask=True):
    ...
    device = edge_index.device
    subset_idx = torch.tensor(tonp(subset_idx),
                              dtype=torch.long,
                              device=device)
    ...
    if subset_newID is None:
        ...
        subset_newID = torch.tensor(tonp(subset_newID),
                                    dtype=torch.long,
                                    device=device)
...
def cal_recall(pos_score_1D, neg_score_1D, topk=None):
    ...
    labels = np.concatenate(
        [np.ones(len(pos_score_1D)),
         np.zeros(len(neg_score_1D))])
    arr = np.asarray([scores, labels]).T  # shape=[N,2]
    sortarr = np.asarray(sorted(list(arr), key=lambda x: x[0],
                                reverse=True))  # [N,2]
...
def set_arch_configs(args):
    ...
    _neurons_proj2linkp = [
        32
    ]  # last dim of this is the projected emb for linkp. can also directly use dim_commonEmb
    args.TeacherGNN.neurons_proj2class = [
        args.dim_commonEmb
    ] + _neurons_proj2class_ + [args.num_classes_bkup]
    args.TeacherGNN.neurons_proj2linkp = [args.dim_commonEmb
                                          ] + _neurons_proj2linkp
    ...
    args.StudentBaseMLP.dims_in_out = [
        args.num_feats_bkup, args.num_classes_bkup
    ]
...
def gen_rec_for_table1_stats(N_nodes, degs):
    recs = [
        ...
        np.mean(degs),
        np.median(degs),
        sum(np.asarray(degs) == 0) / N_nodes * 100
    ]
    return recs
...
def save_graph_analyze(N_nodes, data, use_special_split):
    ...
    if not use_special_split:
        ...
        data.small_deg_mask = torch.tensor([False] * N_nodes,
                                           device=data.x.device)
        data.large_deg_mask = torch.tensor([False] * N_nodes,
                                           device=data.x.device)
        ...
        data.zero_deg_mask = torch.tensor([False] * N_nodes,
                                          device=data.x.device)
        data.small_deg_mask = torch.tensor([False] * N_nodes,
                                           device=data.x.device)
        data.large_deg_mask = torch.tensor([False] * N_nodes,
...
def craft_isolation_v2(data):
    ...
    edge_index_crafted = torch.tensor(edge_index_crafted).t().to(
        data.edge_index.device)
    print(
        f'removed < {cnt} > edge; shape change: {data.edge_index.shape} ›› {edge_index_crafted.shape}'
    )
    print('-' * 20, '\n\n')
...
def linkp_loss_eva(h_emb, t_emb, nh_emb, nt_emb):
    ...
    label = th.cat(
        [th.full((pos_score.shape), 1.0),
         th.full((neg_score.shape), 0.0)]).to(score.device)
...
def cal_MRR(pos_score, neg_score):
    ...
    pos_neg = torch.cat([
        pos_score.reshape(-1, 1), neg_score[:drop_end].reshape(
            len(pos_score), num_neg_per_pos)
    ],
                        dim=1)

    _, indices = th.sort(pos_neg, dim=1, descending=True)
    indices_correct = th.nonzero(
        indices == 0, as_tuple=False
    )  # get all locations where value is zero (all rows treated equally)
    rankings = indices_correct[:, 1].view(
        -1
    ) + 1  # indices[:, 0] is the row-coordinate, indices[:, 1] is the column-coordinate
...
def run_pureLP(res):
    ...
    lp_dict = {
        ...
        'idxs': [
            'intersection'
        ],  # these is used for selecting res.split_idx: res.split_idx is a dict, key is whatever inside this 'idxs', value is the node indices (1D integer vec), these nodes will be the source, to propagate to other nodes
    ...
    if res.args.LP_use_softmax:
        ...
        res.out = general_outcome_correlation(
            adj=res.adj,
            y=res.embs_LPGraph,
            alpha=res.lpStep_alpha,
            num_propagations=res.lpStep_num_propagations,
            post_step=lambda x: torch.clamp(x, 1e-6, 1),
...
def analyze_LP(res):
    ...
    print(
        f'\t embs_pre.shape={embs_pre.shape}, embs_after.shape={embs_after.shape}'
    )
...
def getMLP(neurons,
    ...
    for i in range(n - 1):
        ...
        nn_list.extend([
            nn.Linear(neurons[i], neurons[i + 1], bias=bias), norm,
            activation(),
            nn.Dropout(dropout)
        ])
...
def save_model(net, cwd):  # June 2021
    torch.save(net.state_dict(), cwd + '.pth')
    print(f"‹‹‹‹‹‹‹---  Saved @ :{cwd}\n\n\n")
...
def plot_many(arr_list,
    ...
    for i in range(len(arr_list)):
        plt.plot(arr_list[i],
                 random_line_marker(have_line_yon=have_line_yon,
                                    marker_size_bos=marker_size_bos),
                 label=legends[i],
                 linewidth=1.2,
                 markersize=5)
    plt.legend()
...
class graphUtils:
    ...
    edge_index = example_edge_index = torch.tensor(
        [[0, 0, 1, 1, 1, 2], [0, 1, 0, 1, 2,
                              2]])  # 3 nodes, 6 edges; not symmetric.
    ...
    edge_index_bipt = example_edge_index_bipt = torch.tensor(
        [[0, 0, 1, 1, 1, 2], [3, 4, 3, 4, 5, 5]])  # num_nodes=(3,6).
    ...
    @staticmethod
    def edge_index_to_A(edge_index,
                        num_nodes=None,
                        edge_weight=None,
                        want_tensor=0):
        ...
        def get_A(edge_weight, edge_index):
            if not want_tensor:
                ...
                A = SparseTensor(row=edge_index[1],
                                 col=edge_index[0],
                                 value=edge_weight,
        ...
        if len(edge_weight.shape) == 1:
            ...
            return [
                get_A(edge_weight[:, d], edge_index)
    ...
    def A_to_edge_index(As):
        ...
        if type(As) is list:
            ...
            edge_weight = np.array(
                [tonp(As[i].coalesce().values()) for i in range(len(As))]).T
        else:
    ...
    def edge_index_to_sparse_adj(edge_index, num_nodes=None, edge_weight=None):
        ...
        adj = torch.sparse_coo_tensor(edge_index,
                                      ...
                                      device=edge_index.device).float()
        return adj

    @staticmethod
    def edge_index_to_dense_numpy(edge_index,
                                  num_nodes=None,
                                  edge_weight=None):
        ...
        adj = torch.sparse_coo_tensor(
            ...
            v,
            size=(num_nodes, num_nodes),
            device=edge_index.device).float().numpy()
        return adj
    ...
    def crop_adj_to_subgraph(adj_mtx, subset_idx):
        ...
        edge_index, edge_attr = graphUtils.subgraph(subset_idx,
                                                    edge_index,
                                                    edge_attr,
                                                    relabel_nodes=True,
                                                    num_nodes=adj_mtx.shape[0])
        n2 = len(subset_idx)
        adj2 = torch.sparse_coo_tensor(edge_index,
                                       edge_attr,
                                       size=(n2, n2),
                                       device=adj_mtx.device).float()
        return adj2
